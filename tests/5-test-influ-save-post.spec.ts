...
test('test-influ-save-post', async ({ page }) => {
  ...
  await page.getByText('งานที่ต้อง Post จริง').click({ delay: 1000 });
  await page.waitForTimeout(1000);
  await page.getByRole('button', { name: 'กดส่งแบบร่าง' }).click({ delay: 1000 });
  await page.getByPlaceholder('แปะลิงค์ของโพสต์').click({ delay: 500 });
  await page.getByPlaceholder('แปะลิงค์ของโพสต์').fill('https://www.facebook.com/');
  await page.waitForTimeout(500);
